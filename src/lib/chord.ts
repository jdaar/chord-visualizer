...
export function get_chord_type(chord: Chord): ChordType {
    let chord_intervals = chord['intervals'].slice(0, 2)
    let chord_type = chord_types.filter(type => intervals[type].every(note_interval => chord_intervals.includes(note_interval)))
    return chord_type[0]
}

export function chord_to_string(chord: Chord): ChordString {
    let is_triad = chord['intervals'].length < 3
    let chord_string: ChordString;
    if (is_triad) {
        chord_string = `${chord['base_note']}${get_chord_type(chord)}`
    } else {
        let extension = chord['intervals'][chord['intervals'].length - 1]
...
export function interval_to_note(base_note: Note, interval: number): Note {
    let note_index = notes.indexOf(base_note)
    let is_another_octave = note_index + interval >= notes.length
    return is_another_octave ? notes[note_index + interval - notes.length] : notes[note_index + interval]
}
